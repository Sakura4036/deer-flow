...
每个智能体都可以访问针对其角色优化的特定工具，并在 LangGraph 框架内运行
   - **router (路由器)**：拆解任务并将其分配给适当的研究员
   - **web研究员 (Researcher)**：使用网络搜索引擎、爬虫甚至 MCP 服务等工具进行网络搜索和信息收集。
   - **文献研究员 (Literature Researcher)**：使用文献数据库和学术搜索引擎进行文献研究。
   - **专利研究员 (Patent Researcher)**：使用专利数据库和搜索引擎进行专利研究。
   - **编码研究员(Code Researcher)**：使用 Python REPL 工具处理代码分析、执行和技术任务。
   - **总结员 (Summarizer)**：总结和提炼研究结果
     - 处理来自不同研究员的输入
     - 生成简明的摘要和关键要点
